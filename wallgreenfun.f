     ...
     + line5,line6,line7,tempsum
      ...
      wallgnfunthree=2.0/rdist**3*delta(i,j)*y3*rij(3)-
     + 2.0*y3/rdist**3*(delta(j,3)*rij(i)+delta(i,3)*rij(j))+
     + 6.0/rdist**5*rij(i)*rij(j)*y3*rij(3)-
     + 4.0/rdist**3*delta(i,j)*delta(j,3)*y3*rij(3)+
     + 4.0*(y3)/rdist**3*(delta(3,j)*rij(i)+
     + delta(j,3)*delta(i,3)*rij(j))-
     + 12.0/rdist**5*delta(3,j)*rij(i)*rij(j)*y3*rij(3)
     
      wallgnfunthree=-2.0*y3*(delta(i,j)-2*delta(3,i)*delta(3,j))*
     + (-delta(3,j)*rij(i)/rdist**3+delta(3,i)*rij(j)/rdist**3+
     + rij(3)*delta(i,j)/rdist**3-3.0*rij(3)*rij(i)*rij(j)/rdist**5)
