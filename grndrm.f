...
c    ******************************************************************
	  ...
	  double precision      r(n3), a, rmove(np3), t, ravg, rsq, 
...
c
	  ...
	  do i=1,np
		...
		do  j=1,3
		  ...
		  rsq = rsq + ( r(nxyz(j,i)) - rmove(nxyz(j,i)) )**2
	  ...
	  ravg = ravg / np		
c
c	construct the far-field resistance tensor. a mobility tensor is 
c	constructed for np particles and nw wall elements (np+nw) spheres
c	and inverted to get the far-field resistance tensor
c
c	to avoid the frequent expensive calculation of the far-field tensor,
c	calculations are done only if the average particle displacements are 
c	equal to or greater than the particle radius.
c
c	  if(ravg.ge.a/10.0)then
c
...
c			call invert (Gact, np3)
		call farfldres(rinfi,  r, rimage, nxyz)
...
c		  ry = ry - anint(ry / boxlen) * boxlen
		  ...
		  if(rsep .lt. 6.0*a)then 	
		    ...
		    call ppinf(rabinf, p1, p2, r,rimage, nxyz)
